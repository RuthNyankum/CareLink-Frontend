...
const HowItWorks = () => {
  return (
    <div className="bg-gray-100 py-12">
      ...
      <div className="flex flex-col items-center max-w-5xl mx-auto space-y-8 mb-7">
        {/* Step Group 1 */}
        <div className="flex flex-col md:flex-row md:justify-between md:space-x-16">
          {steps
            ...
            .map(({ icon: Icon, color, title, description }, index) => (
              <div
                key={index}
                className="relative flex flex-col items-center text-center space-y-4 md:space-y-2"
              >
                <Icon className={`h-10 w-10 ${color} mb-2`} />
                <h2 className="text-2xl font-semibold">{title}</h2>
                <p className="text-gray-600 text-lg">{description}</p>
                ...
                {index < 2 && (
                  <div className="absolute right-[-50px] top-1/2 transform -translate-y-1/2 hidden md:block">
                    <div className="w-12 border-t-4 border-primary"></div>
                    <div className="border-t-4 border-r-4 border-primary w-4 h-4 transform rotate-45 -translate-y-2.5 translate-x-8"></div>
                  </div>
                )}
                ...
                {index === 2 && (
                  <div className="absolute left-1/2 top-full transform -translate-x-1/2 -translate-y-1 hidden md:block">
                    <div className="w-2 border-l-4 border-primary h-16"></div>
                    <div className="w-4 h-4 border-l-4 border-b-4 border-primary transform -rotate-45 -translate-x-1.5 -translate-y-4"></div>
                  </div>
                )}
